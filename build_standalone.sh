...
DELTARUNE_DDD_CH1_KRISTAL_REF="${DELTARUNE_DDD_CH1_KRISTAL_REF:-f62afea63ccab02f468c24ac0d096bd8a2c9aa81}"
...
ensure_kristal() {
    ...
    if ! git -C "$DELTARUNE_DDD_CH1_KRISTAL_DIR" rev-parse --verify --quiet "${DELTARUNE_DDD_CH1_KRISTAL_REF}^{commit}" >/dev/null; then
        git -C "$DELTARUNE_DDD_CH1_KRISTAL_DIR" fetch --depth 1 origin "${DELTARUNE_DDD_CH1_KRISTAL_REF}"
    fi
